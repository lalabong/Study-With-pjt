...
import getTopRunningSchedule from '@api/schedule/getTopRunningSchedule';

import { SCHEDULE_ERROR_MESSAGES, USER_ERROR_MESSAGES, USER_QUERY_KEYS } from '@constants/index';

interface UseTopRunningScheduleQueryParams {
  userId: string;
  date: string;
  enabled?: boolean;
}

export const useTopRunningScheduleQuery = ({
  userId,
  date,
  enabled = true,
}: UseTopRunningScheduleQueryParams) => {
  const query = useQuery({
    queryKey: [USER_QUERY_KEYS.USER_TOP_RUNNING_SCHEDULE, userId, date],
    queryFn: async () => {
      const response = await getTopRunningSchedule({ userId, date });
    ...
    if (query.error) {
      ...
      switch (errorCode) {
        ...
        default:
          ...
          console.error('최상단 진행중 일정 조회 중 오류 발생:', error);
      }
    }
  }, [query.error]);
